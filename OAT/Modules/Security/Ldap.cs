...
namespace OAT.Modules.Security
{
	public class Ldap
	{
		...
		public static bool Login(string username, string password, string IP, bool WithError = true)
		{
			try
			{
				...
				username = OperatingSystem.IsWindows() ? username : $"{Configurator.config.ldap_domain}\\{username}";
				var conn = new LdapConnection(new LdapDirectoryIdentifier(Configurator.config.ldap_IP, Configurator.config.ldap_port), new NetworkCredential(username, password), authType);
				conn.Bind();
				return true;
			}
			catch (Exception ex)
		...
		{
			using var ldap = new LdapConnection(new LdapDirectoryIdentifier(Configurator.config.ldap_IP, Configurator.config.ldap_port));

			ldap.SessionOptions.ProtocolVersion = 3;
			ldap.AuthType = AuthType.Basic;
			ldap.Bind(new NetworkCredential(Configurator.config.ldap_login, Configurator.config.ldap_password));

			var search = new SearchRequest
			{
				DistinguishedName = $"DC={Configurator.config.ldap_domain},DC={Configurator.config.ldap_zone}",
