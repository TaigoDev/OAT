...
public class FilesController : Controller
{
	...
	[HttpGet("changes/{building}/09876635187285765187736186318263782/download")]
	[NoCache]
	public async Task<IActionResult> DownloadChanges(string building)
	{
		...
		return File(await System.IO.File.ReadAllBytesAsync(path),
			"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"{building}Changes.xlsx");
	}
	...
	[HttpGet("api/sessions/{building}/files")]
	[NoCache]
	...
	public async Task<IActionResult> DownloadSessionFile(string building, string filename)
	{
		...
		return File(await System.IO.File.ReadAllBytesAsync(file),
			"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", $"{building}Sessions.xlsx");
	}
	...
	[HttpGet("api/practice/{building}/files")]
	[NoCache]
	...
	{
		var file = Path.Combine(Directory.GetCurrentDirectory(), "Resources", "practice", building,
			StringUtils.ConvertHexToString(filename));
		if (!System.IO.File.Exists(file))
			return StatusCode(StatusCodes.Status404NotFound);
		return File(await System.IO.File.ReadAllBytesAsync(file), MimeTypeMap.GetMimeType(Path.GetExtension(file)),
			Path.GetFileName(StringUtils.ConvertHexToString(filename)));
	}
