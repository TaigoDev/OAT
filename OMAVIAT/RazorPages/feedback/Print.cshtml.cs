...
public class PrintModel : PageModel
{
	...
	public void OnGet([FromQuery] int purpose, [FromQuery] int Voluntary_Purpose, [FromQuery] string documentId,
		[FromQuery] string documentDate,
		[FromQuery] string studentFullName, [FromQuery] string group, [FromQuery] string FullName,
		[FromQuery] int summa, [FromQuery] string email,
		[FromQuery] string phone)
	{
		if (purpose == 4)
		{
			...
			M_summa = summa;
			var qrData =
				$"ST00012|Name=Министерство финансов Омской области (БПОУ «Омавиат» л/с 010220608)|PersonalAcc=03224643520000005201|BankName=Отделение Омск Банка России//УФК по Омской области г. Омск|BIC=015209001|CorrespAcc=0|PayeeINN=5504000055|KPP=550401001|" +
		...
		else
		{
			...
			M_Purpose =
				$"{GetPurpose(purpose)} по дог.№ {contract.documentId.Replace(" ", "")}, {contract.studentFullName} гр.№ {contract.Group}";
