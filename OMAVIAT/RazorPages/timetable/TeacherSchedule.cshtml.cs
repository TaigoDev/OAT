...
public class TeacherScheduleModel : PageModel
{
	...
	public void OnGet(string building, string fullname)
	{
		...
		if (corpus == null)
			return;
		teacher = ScheduleReader.teachers.FirstOrDefault(e =>
			e.name.ToSearchView() == fullname.ToSearchView() && e.Corpus == (Corpus)result - 1);
	}
	...
	public List<ScheduleLesson?> GetAllLessonsByNumber(int id, ScheduleWeek week)
	{
		...
		foreach (var day in week.Days.OrderBy(e => e.Type))
		{
			...
			switch (lesson.Count)
			{
				...
				default:
				{
					if (lesson.Count != 0) lessons[day.Type - 1] = lesson.First();
	...
	{
		return week.Days.Max(e => e.lessons.Count == 0 ? 0 : e.lessons.Max(l => l.Id));
	}

	public int GetMinLessonsInDay(ScheduleWeek week)
	{
		return week.Days.Min(e => e.lessons.Count == 0 ? 100 : e.lessons.Min(l => l.Id));
	}
}